...
// Verzamelen van factuurgegevens uit het POST-formulier en uitvoeren van vereiste validatie en filtering.
		$name = $_POST['name'];
		...
		// Controleren of er items in de winkelwagen zijn.
		$varify_cart = $conn->prepare("SELECT * FROM `cart` WHERE user_id=?");
		$varify_cart->execute([$user_id]);
		// Verwerken van bestelling met betrekking tot een specifiek product.
		if (isset($_GET['get_id'])) {
			$get_product = $conn->prepare("SELECT * FROM `products` WHERE id=? LIMIT 1");
			$get_product->execute([$_GET['get_id']]);
			if ($get_product->rowCount() > 0) {
				while($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)){
					// Invoegen van de bestelling in de database en doorverwijzen naar de orderpagina.
			...
			// Verwerken van de bestelling met betrekking tot de items in de winkelwagen.	
			...
			if ($insert_order) {
				// Leegmaken van de winkelwagen na het plaatsen van de bestelling.
...
<body>
	...
	<!-- Hoofdinhoud van de pagina, inclusief banner, navigatie en besteloverzicht. -->
	<div class="main">
		<div class="banner">
                ...
                <div class="row">
                	...
                	<div class="summary">
                		...
                		<div class="box-container">
                			<?php 
							 // Controleer of een specifieke product-ID is ingesteld in de URL 
                				$grand_total=0;
                				if (isset($_GET['get_id'])) {
                			...
                			<?php 
									...
									// Als er geen specifieke product-ID is ingesteld, toon items uit de winkelwagen van de gebruiker
                					$select_cart = $conn->prepare("SELECT * FROM `cart` WHERE user_id=?");
                					$select_cart->execute([$user_id]);
									 // Controleer of er items aanwezig zijn in de winkelwagen van de gebruiker
                					if ($select_cart->rowCount()>0) {
                						while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)){
											// Haal details op van het product in de winkelwagen
                							$select_products=$conn->prepare("SELECT * FROM `products` WHERE id=?");
                							$select_products->execute([$fetch_cart['product_id']]);
                							$fetch_product = $select_products->fetch(PDO::FETCH_ASSOC);
											 // Bereken de subtotaal voor elk item in de winkelwagen
                							$sub_total= ($fetch_cart['qty'] * $fetch_product['price']);
                							$grand_total += $sub_total;
											 // Toon informatie over het winkelwagenitem in een flexibele container
                			...
                			<?php 
										 ...
										 // Toon een bericht als de winkelwagen van de gebruiker leeg is
